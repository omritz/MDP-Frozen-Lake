...
def one_step_lookahead(mdp, state, V, gamma, r):
    A = np.zeros(mdp.nActions)
    for a in range(mdp.nActions):
        for i in range(4):
            prob, next_state, re, done = mdp.step(state, a, i, r)
            # print('Action: %s, i: %s, State: %s, Probability: %s, Next state: %s, Reward: %s, Done: %s'
            #       % (a, i, state, prob, next_state, reward, done))
            A[a] += prob * (re + gamma * V[next_state-1])
    return A


def value_iteration(mdp, theta, gamma, reward, max_iterations=10000):
    ...
    for i in range(max_iterations):
        ...
        for s in range(mdp.nStates):
            ...
            A = one_step_lookahead(mdp, s, V, gamma, reward)
...
def policy_eval(policy, mdp, theta, gamma, reward, max_iterations=10000):
    ...
    while True:
        ...
        for s in range(mdp.nStates):
            ...
            for a, action_prob in enumerate(policy[s]):
                ...
                for i in range(4):
                    prob, next_state, rew, done = mdp.step(s, a, i, reward)
                    # Calculate the expected value
                    v += action_prob * prob * (rew + gamma * V[next_state-1])
...
def policy_improvement(mdp, policy, V):
    ...
    for s in range(mdp.nStates):
        ...
        action_values = one_step_lookahead(mdp, s, V, gamma, reward)
...
def policy_iteration(mdp, theta, gamma, reward):

    # Start with a random policy
    policy = np.ones([mdp.nStates, mdp.nActions]) / mdp.nActions
    while True:
        # Evaluate the current policy
        V = policy_eval(policy, mdp, theta, gamma, reward)
        policy_stable = policy_improvement(mdp, policy, V)
        mdp.plot_value(V)
